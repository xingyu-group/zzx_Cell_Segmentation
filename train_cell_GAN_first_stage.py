...
def train_on_device(args):
    ...
    main_path = '/home/zhaoxiang/NeurIPS-CellSeg/data/Train_Pre_3class'
    ...
    if args.model == 'ws_skip_connection':
        ...
        model_1 = ReconstructiveSubNetwork(in_channels=1, out_channels=3).cuda()
        model_2 = ReconstructiveSubNetwork(in_channels=3, out_channels=1).cuda()
    elif args.model == 'DRAEM_discriminitive':
        model_1 = DiscriminativeSubNetwork(in_channels=3, out_channels=3).cuda()
        model_2 = DiscriminativeSubNetwork(in_channels=3, out_channels=3).cuda()
    ...
    model_1 = torch.nn.DataParallel(model_1, device_ids=[0, 1])
    ...
    if args.resume_training:
        model_1.load_state_dict(torch.load(ckp_path)['model_1'])
        model_2.load_state_dict(torch.load(ckp_path)['model_2'])
        last_epoch = torch.load(ckp_path)['epoch']
        
    train_data = MVTecDataset(root=main_path, transform = test_transform, gt_transform=gt_transform, phase='train', args = args)
    val_data = MVTecDataset(root=main_path, transform = test_transform, gt_transform=gt_transform, phase='test', args = args)
    test_data = MVTecDataset(root=main_path, transform = test_transform, gt_transform=gt_transform, phase='test', args = args)
    ...
    test_dataloader = torch.utils.data.DataLoader(test_data, batch_size = args.bs, shuffle = False)
        
    loss_l1 = torch.nn.L1Loss()
    optimizer = torch.optim.Adam(list(model_1.parameters()) + list(model_2.parameters()), lr = args.lr)
