...
class CutPasteNormal(CutPaste):
    ...
    def __call__(self, img):
        ...
        gt = Image.new("L", (h,w))
        mask_h, mask_w = patch.size[0], patch.size[1]
        mask = Image.new("L", (mask_h,mask_w), 255)
        gt.paste(mask, (to_location_w, to_location_h))
...
class CutPasteScar(CutPaste):
    ...
    def __call__(self, img):
        ...
        mask_h, mask_w = patch.size[0], patch.size[1]
        mask = Image.new("L", (mask_h,mask_w), 255)
        mask = mask.rotate(rot_deg,expand=True)
        gt.paste(mask, (to_location_w, to_location_h))
