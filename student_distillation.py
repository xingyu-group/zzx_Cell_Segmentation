...
def train_on_device(args):
    ...
    optimizer = torch.optim.Adam(list(decoder.parameters())+list(bn.parameters()), lr=args.lr, betas=(0.5,0.999))
    
    for epoch in range(args.epochs):
        # dice_value, auroc_px, auroc_sp, aupro_px = evaluation(args, classifier, decoder, bn, test_dataloader, epoch, loss_function, run_name)
        # result_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'outputs', run_name, 'results.txt')
        # print('Pixel Auroc:{:.3f}, Sample Auroc{:.3f}, Pixel Aupro{:.3f}, Dice{:3f}'.format(auroc_px, auroc_sp, aupro_px, dice_value))
        
        # ckp_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'checkpoints', run_name+'student.pth')
        # torch.save({'bn': bn.state_dict(),
        #             'decoder': decoder.state_dict()}, ckp_path)
    
        # with open(result_path, 'a') as f:
        #     f.writelines('Epoch:{}, Pixel Auroc:{:.3f}, Sample Auroc{:.3f}, Pixel Aupro{:.3f}, Dice:{:3f}'.format(epoch, auroc_px, auroc_sp, aupro_px, dice_value))     
        ...
        for img, aug in train_dataloader:
            ...
            outputs = decoder(bn(embedding))
            loss = loss_function(embedding, outputs)
        ...
        if (epoch) % 10 == 0:
            ...
            with open(result_path, 'a') as f:
                f.writelines('Epoch:{}, Pixel Auroc:{:.3f}, Sample Auroc{:.3f}, Pixel Aupro{:.3f}, Dice:{:3f} \n'.format(epoch, auroc_px, auroc_sp, aupro_px, dice_value))     
